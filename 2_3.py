...
def compute_subprob(node, i, theta, beta, k, memory_cache):
    if str(int(node.name))+str(i)+str(beta[int(node.name)]) in memory_cache:
        return memory_cache[str(int(node.name))+str(i)+str(beta[int(node.name)])]
    ...
    if len(node.descendants) == 0:
        ...
        left_child = node.descendants[0]
        right_child = node.descendants[1]
        left=0
        right=0


        for j in range(k):
            left += theta[int(left_child.name)][i][j] * compute_subprob(left_child, j, theta, beta, k, memory_cache)
            right += theta[int(right_child.name)][i][j] * compute_subprob(right_child, j, theta, beta, k, memory_cache)
        value = left * right

    memory_cache[str(int(node.name))+str(i)+str(beta[int(node.name)])] = value
...
def calculate_likelihood(tree_topology, theta, beta):
    ...
    likelihood = 0
    for i in range(k):
        likelihood += compute_subprob(t.root, i, theta, beta, k, memory_cache) * theta[0][i]
        memory_cache.clear()
