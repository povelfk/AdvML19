...
# 1) kollar om noden ar ett lov
#       - om lov, kollar vilket lov (namn/index)
#       - om inte lov, skicka till 

#@lru_cache(maxsize=100000)
def compute_subprob(node, i, theta, beta, k):
    ...
    if len(node.descendants) == 0:
        ...
        for j in range(k):
            left += theta[int(left_child.name)][i][j] * compute_subprob(left_child, j, theta, beta, k)
            right += theta[int(right_child.name)][i][j] * compute_subprob(right_child, j, theta, beta, k)
...
def calculate_likelihood(tree_topology, theta, beta, t):
    """
    ...
    likelihood = 0
    for i in range(k):
        likelihood += compute_subprob(t.root, i, theta, beta, k) * theta[0][i]
